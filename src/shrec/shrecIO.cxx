...
namespace {
  ...
  typedef enum {
    ...
    SELECT_MOD3_PARAM, SELECT_MOD6_PARAM, SELECT_BY_DIST_PARAM,
    VERSION_PARAM, HELP_PARAM, HELP_OUTPUT_PARAM, HELP_TESTING_PARAM,
    LIST_ALL_OPTIONS_PARAM,
    OFF_PARAM, IV_PARAM,
    OUTPUT_FILENAME_PARAM, STDOUT_PARAM, NOWRITE_PARAM, 
    USEV_IN_OUTFNAME_PARAM,
  ...
  const char * parameter_string[] =
    { "-subsample",
      ...
      "-select_mod3", "-select_mod6", "-select_by_dist",
      "-version", "-help", "-help_output", "-help_testing",
      "-list_all_options", "-off", "-iv", 
      "-o", "-stdout", "-nowrite", "-usev_in_outfname",
  ...
  void lindstrom_deprecated()
  {
    ...
    cerr << "   Program SHREC always creates 3x3 matrix from the gradients"
  ...
  (const PARAMETER param, INPUT_INFO & input_info)
  {
    ...
    case MERGE_PARAM:
      input_info.flag_merge = true;
      break;

    case NO_MERGE_PARAM:
      input_info.flag_merge = false;
      break;
    ...
    case MAP_EXTENDED_PARAM:
      ...
      input_info.is_flag_map_extended_set = true;
      break;
    ...
    case SELECT_MOD6_PARAM:
      ...
      input_info.is_cube_selection_method_set = true;
      break;

    case SELECT_BY_DIST_PARAM:
      input_info.flag_select_mod6 = false;
      input_info.flag_select_mod3 = false;
      input_info.is_cube_selection_method_set = true;
      break;
    ...
    case USEV_IN_OUTFNAME_PARAM:
      input_info.flag_use_isovalue_in_output_filename = true;
      break;
    ...
    case HELP_PARAM:
      input_info.flag_output_help = true;
      break;

    case LIST_ALL_OPTIONS_PARAM:
      input_info.flag_list_all_options = true;
      break;
   ...
   const char * value_string, INPUT_INFO & input_info)
  {
    ...
    case MAX_EIGEN_PARAM:
      ...
      input_info.is_max_small_eigenvalue_set = true;
      break;

    case MAX_DIST_PARAM:
      input_info.max_dist = 
        get_option_float(option_string, value_string);
      input_info.is_max_dist_set = true;
      break;
    ...
    case GRAD_S_OFFSET_PARAM:
      input_info.grad_selection_cube_offset = 
        get_option_float(option_string, value_string);
      input_info.is_grad_selection_cube_offset_set = true;
      break;
...
bool SHREC::parse_command_option
...
void SHREC::parse_isovalue_and_filename
...
void SHREC::set_input_info_defaults(INPUT_INFO & input_info)
{
  ...
  if (!input_info.is_cube_selection_method_set) {
    input_info.flag_select_mod6 = shrec_defaults.flag_select_mod6;
    input_info.flag_select_mod3 = false;
  }

  if (!input_info.is_flag_map_extended_set) {
    input_info.flag_map_extended = shrec_defaults.flag_map_extended;
  }

  if (!input_info.is_max_dist_set) {
    input_info.max_dist = shrec_defaults.max_dist;
  }

  if (!input_info.is_max_small_eigenvalue_set) {
    input_info.max_small_eigenvalue = shrec_defaults.max_small_eigenvalue;
  }

  if (!input_info.is_grad_selection_cube_offset_set) {
    input_info.grad_selection_cube_offset = 
      shrec_defaults.grad_selection_cube_offset;
  }

  if (!input_info.is_conflict_set && input_info.flag_merge) {
...
// Check input_info.
void SHREC::check_input_info(const INPUT_INFO & input_info)
{
...
(int argc, char **argv, INPUT_INFO & input_info)
{
  ...
  while (iarg < argc) {
    ...
    if (!parse_command_option(argc, argv, iarg, next_arg, input_info) ||
      ...
      if (argv[iarg][0] == '-') {
        PARAMETER param = get_parameter_token(argv[iarg]);
        if (param == HELP_OUTPUT_PARAM) { help_output(argv[0]); }
        if (param == HELP_TESTING_PARAM) { help_testing(argv[0]); }
      }
      break; 
    }
  ...
  if (input_info.flag_output_help) 
    { help(argv[0], input_info.flag_list_all_options); }
...
void SHREC::output_dual_isosurface
(const OUTPUT_INFO & output_info, const SHREC_DATA & shrec_data,
 const DUAL_ISOSURFACE & dual_isosurface, const ISOVERT & isovert, 
 const SHREC_INFO & shrec_info, IO_TIME & io_time)
{
  if (!output_info.use_stdout && !output_info.flag_silent) {
    report_iso_info3D
      (output_info, shrec_data, dual_isosurface, shrec_info);

    report_isovert_info(output_info, shrec_data.ScalarGrid(), isovert);

    if (output_info.flag_output_neighbors) {
      report_far_neighbors
        (output_info, shrec_data.ScalarGrid(), isovert);
    }
  }
  ...
  if (!output_info.nowrite_flag) {
    ...
    if (output_info.flag_store_isovert_info) {
      write_isovert_info(output_info, shrec_info.sharpiso.vertex_info);
	}
  }
...
void SHREC::rescale_vertex_coord
...
void SHREC::report_num_cubes
(const SHARPISO_GRID & full_scalar_grid, const INPUT_INFO & input_info,
 const SHREC_DATA & shrec_data)
{
  const int num_grid_cubes = full_scalar_grid.ComputeNumCubes();
  const int num_cubes_in_shrec_data =
    shrec_data.ScalarGrid().ComputeNumCubes();
  ...
  if (!input_info.use_stdout && !input_info.flag_silent) {
    ...
    if (input_info.flag_subsample) {
      ...
      cout << num_grid_cubes << " grid cubes.  "
           << num_cubes_in_shrec_data << " subsampled grid cubes." << endl;
    }
    else if (input_info.flag_supersample) {
      // supersample grid
      cout << num_grid_cubes << " grid cubes.  "
           << num_cubes_in_shrec_data << " supersampled grid cubes." << endl;
    }
    else {
...
(const SHREC_PARAM & shrec_param)
{
  ...
  case GRADIENT_POSITIONING:
    {
      ...
      GRAD_SELECTION_METHOD grad_selection_method =
        shrec_param.GradSelectionMethod();
  ...
  if (shrec_param.flag_merge) {
    ...
    if (shrec_param.flag_allow_conflict) {
  ...
  if (vpos_method != CUBECENTER && vpos_method != EDGEI_INPUT_DATA) {
    ...
    if (shrec_param.use_sharp_edgeI) {
  ...
  if (shrec_param.allow_multiple_iso_vertices) {
    ...
    if (shrec_param.flag_resolve_ambiguous_facets &&
        !shrec_param.flag_merge) {
      cout << "Resolve ambiguous facets." << endl;
    }
    else if (shrec_param.flag_separate_neg) {
  ...
  if (shrec_param.flag_select_mod6) {
    cout << "Using mod 6 selection of sharp cubes." << endl;
  }
  else if (shrec_param.flag_select_mod3) {
    cout << "Using mod 3 selection of sharp cubes." << endl;
  }
  else {
    cout << "Selecting sharp cubes based on distance from isovert to cube centers."
         << endl;
  }

  if (shrec_param.flag_map_extended) {
    cerr << "Extend cube merging beyond 3x3x3 subgrids." << endl;
  }
  else {
    cerr << "Restrict cube merging to 3x3x3 subgrids." << endl;
  }
...
void SHREC::report_iso_info3D
(const OUTPUT_INFO & output_info, const SHREC_DATA & shrec_data,
 const DUAL_ISOSURFACE & dual_isosurface,
 const SHREC_INFO & shrec_info)
{
  const char * indent4 = "    ";
  ...
  VERTEX_INDEX num_grid_cubes = shrec_info.grid.num_cubes;
  VERTEX_INDEX num_non_empty_cubes = shrec_info.scalar.num_non_empty_cubes;
  ...
  if (output_info.flag_output_alg_info) {
    ...
    if (vpos_method == GRADIENT_POSITIONING ||
      ...
      cout << endl;
      if (!output_info.flag_merge) {
        cout << "  # of conflicts: "
             << shrec_info.sharpiso.num_conflicts << endl;
      }
      cout << "  # of sharp corners: "
           << shrec_info.sharpiso.num_sharp_corners << endl;
      cout << "  # of isosurface vertices on sharp edges: "
           << shrec_info.sharpiso.num_sharp_edges << endl;
      cout << "  # of smooth isosurface vertices: "
           << shrec_info.sharpiso.num_smooth_vertices << endl;
      if (!output_info.flag_merge && output_info.use_Linf_dist) {
        cout << "  # of vertices at min Linf distance to cube center: "
             << shrec_info.sharpiso.num_Linf_iso_vertex_locations << endl;
      }

      if (output_info.flag_merge) {
        cout << "  # of merged isosurface vertices: "
             << shrec_info.sharpiso.num_merged_iso_vertices << endl;
      }
        ...
        if (!output_info.flag_merge) {
          if (output_info.flag_resolve_ambiguous_facets) {
            cout << "  # of non-ambiguous cubes: "
                 << shrec_info.sharpiso.num_cube_not_ambiguous << endl;
            cout << "  # of separate positive cubes: "
                 << shrec_info.sharpiso.num_cube_separate_pos << endl;
            cout << "  # of separate negative cubes: "
                 << shrec_info.sharpiso.num_cube_separate_neg << endl;
            cout << "  # of unresolved ambiguous cubes: "
                 << shrec_info.sharpiso.num_cube_unresolved_ambiguity 
                 << endl;
          }
        }

        cout << "  # of cubes with single isov: "
             << shrec_info.sharpiso.num_cube_single_isov << endl;
        cout << "  # of cubes with multi isov: "
             << shrec_info.sharpiso.num_cube_multi_isov << endl;
        ...
        if (output_info.flag_merge) { 
          ...
          if (output_info.flag_select_split) {
            cout << "  # of cubes changed in selecting isosurface patch splits: "
                 << shrec_info.sharpiso.num_1_2_change << endl;
          }
      ...
      if (output_info.flag_merge) {
        if (output_info.flag_check_disk) {
          cout << "  # of merges blocked by non-disk isosurface patches: "
               << shrec_info.sharpiso.num_non_disk_isopatches << endl;
        }
      }
...
void SHREC::report_active_cubes
(const OUTPUT_INFO & output_info, const SHARPISO_GRID & grid,
 const ISOVERT & isovert)
{
  cout << endl;
...
void SHREC::report_cubes_which_map_to_self
(const OUTPUT_INFO & output_info, const SHARPISO_GRID & grid,
 const ISOVERT & isovert)
{
  cout << endl;
...
void SHREC::report_covered_cubes_which_map_to_self
(const OUTPUT_INFO & output_info, const SHARPISO_GRID & grid,
 const ISOVERT & isovert)
{
  cout << endl;
...
void SHREC::report_cubes_which_map_to
(const OUTPUT_INFO & output_info, const SHARPISO_GRID & grid,
...
/// Report information about isosurface vertices
void SHREC::report_isovert_info
(const OUTPUT_INFO & output_info, const SHARPISO_GRID & grid,
 const ISOVERT & isovert)
{
 ...
 const char * mesh_type_string)
{
  ...
  if ((input_info.VertexPositionMethod() != CUBECENTER &&
    ...
    cout << "    Time to merge identical "
         << mesh_type_string << " vertices: "
         << shrec_time.merge_identical << " seconds." << endl;
...
void SHREC::report_time
(const INPUT_INFO & input_info, const IO_TIME & io_time,
 const SHREC_TIME & shrec_time, const double total_elapsed_time)
{
  ...
  report_shrec_time(input_info, shrec_time, mesh_type_string);
...
namespace {
  ...
  void main_options_msg()
  {
    ...
    cerr << "  [-position {centroid|cube_center|grad3|grad5|grad7|grad9}]"
         << endl;
    cerr << "  [-gradient {gradient_nrrd_filename}]"
         << " [-normal {normal_off_filename}]" << endl;
    cerr << "  [-subsample S] [-max_eigen {max}]" << endl;
    cerr << "  [-trimesh] [-keepv] [-o {output_filename}] [-usev_in_outfname] [-stdout]"
         << endl;
    cerr << "  [-s] [-out_param] [-info] [-nowrite] [-time]"
         << endl;
    cerr << "  [-help] [-version] [-list_all_options]"
         << endl;
  }
  ...
  void testing_options_msg()
  {
    ...
    cerr << "  [-position {gradC|gradCS|gradCD|gradXS|gradIE|gradIES|gradIEDir"
         << endl
         << "                   |gradES|gradEC|gradN|gradNS|gradNIE|gradNIES|gradBIES}]"
         << endl;
    cerr << "  [-merge | -no_merge] [-merge_linf_th <D>]" << endl;
    cerr << "  [-grad2hermite | -grad2hermiteI]" << endl;
    cerr << "  [-select_split]" << endl;
    cerr << "  [-select_mod6 | -select_by_dist]" << endl;
    ...
    cerr << "  [-map_extended | -no_map_extended]" <<endl;
    cerr << "  [-help_testing]" << endl;
  }

  void output_options_msg()
  {
    cerr << "MORE OUTPUT OPTIONS:" << endl;
    ...
    cerr << "  [-write_isov_info]" << endl;
    cerr << "  [-help_output]" << endl;
  }

  void options_msg(const bool flag_list_all_options)
  {
    main_options_msg();
...
void SHREC::usage_error
(const char * command_path, const bool flag_list_all_options)
{
  usage_msg(cerr, command_path);
  options_msg(flag_list_all_options);
  exit(10);
}
...
void help_main_options()
{
  ...
  cout << "  -position grad3|grad5|grad7|grad9:" << endl
       << "     Position isosurface vertices using svd on selected gradients"
       << endl
       << "     in a 3x3x3, 5x5x5, 7x7x7 or 9x9x9 subgrid around cube."
       << endl;
  ...
  cout << "  -out_param: Print shrec parameters." << endl;
...
void help_output_options()
{
  ...
  cout << "  -out_neighbors {\" 222 | 22x | 32x | 33x \"}:" << endl;
  cout << "       Output pairs of selected neighboring cubes at given distances." << endl;
  cout << "       Distances: 222 = (2,2,2). 22x = (2,2,x). 32x = (3,2,x). 33x = (3,3,x)." << endl;
  cout << "       22x |: Output neighbors at distance (2,2,2) (or greater.)"
       << endl;
  ...
  cout << "  -help_output:    Print help on more output options." << endl;
}
...
void help_testing_options()
{
  ...
  cout << "       on selected gradients at endpoints of intersected cube edges."
       << endl;
  cout << "           Select gradients based on direction along edge."
       << endl;
  ...
  cout << "  -normal {normal_off_filename}: Read edge-isosurface intersections"
       << endl
       << "      and normals from OFF file normal_off_filename." << endl;
  cout << "  -merge:    Allow cube merging." << endl;
  cout << "  -no_merge: No cube merging." << endl;
  ...
  cout << "  -max_dist {D}:    Set max Linf distance from cube to isosurface vertex."
       << endl;
  cout << "  -max_mag {max}:  Set maximum small gradient magnitude to max."
       << endl;
  cout << "           Gradients with magnitude below max are ignored." << endl;
  cout << "  -snap_dist {D}:  Snap points within distance D to cube."
       << endl;
  cout << "  -max_grad_dist {D}: Max distance (integer) of gradients to cube. (Default 1.)" << endl;
  cout << "  -gradS_offset {offset}: Set cube offset for gradient selection to offset."
       << endl;
  cout << "           Offset measures distance from cube boundary." << endl;
  cout << "  -min_triangle_angle {A}:  Avoid cube merging which creates triangles" << endl
       << "             with angle less than {A}." << endl;
  cout << "  -min_normal_angle {A}:  Allow cube merging if angle between" << endl
  ...
  cout << "              Suggest using n=16,32,64,... or 2^k for some k."
       << endl;
  cout << "  -trimesh:   Output triangle mesh." << endl;
  cout << "  -map_extended:    Allow cube merging beyond 3x3x3 subgrids (default)." 
       << endl;
  cout << "  -no_map_extended: Restrict cube merging to 3x3x3 subgrids."
       << endl;
  cout << "  -help_testing:    Print help on testing options." << endl;
}

void SHREC::help_output(const char * command_path)
{
...
void SHREC::help_testing(const char * command_path)
{
  usage_msg(cout, command_path);
  cout << endl;
...
void SHREC::help(const char * command_path, const bool flag_list_all_options)
{
  usage_msg(cout, command_path);
  cout << endl;
  cout << "shrec - (SHarp REConstruction)  Construct isosurface with sharp features." << endl;
  cout << endl;
  help_main_options();
...
void SHREC::IO_INFO::Init()
{
  ...
  flag_use_isovalue_in_output_filename = false;
...
void SHREC::IO_INFO::Set(const IO_INFO & io_info)
{
  *this = io_info;
}
...
void SHREC::INPUT_INFO::Init()
{
  Clear();
}

/// Clear input information
void SHREC::INPUT_INFO::Clear()
{
  is_vertex_position_method_set = false;
  is_use_sharp_edgeI_set = false;
  is_conflict_set = false;
  is_cube_selection_method_set = false;
  is_flag_map_extended_set = false;
  is_max_dist_set = false;
  is_max_small_eigenvalue_set = false;
  is_grad_selection_cube_offset_set = false;

  flag_output_help = false;
  flag_list_all_options = false;
...
namespace {
  ...
  (const INPUT_INFO & input_info, const int i)
  {
    ...
    if (input_info.flag_use_isovalue_in_output_filename) {
      ofilename += 
        string(".") + string("isov=") + input_info.isovalue_string[i];
    }
...
void SHREC::set_shrec_data
(const INPUT_INFO & input_info, SHREC_DATA & shrec_data, 
 SHREC_TIME & shrec_time)
{
  PROCEDURE_ERROR error("set_shrec_data");

  if (!shrec_data.IsScalarGridSet()) {
    error.AddMessage
      ("Programming error. Scalar field must be set before set_shrec_data is called.");
    throw error;
  }

  // Set data structures in shrec_data
  shrec_data.Set(input_info);
}

void SHREC::set_input_info
...
void SHREC::set_color_alternating
...
void SHREC::construct_gradient_filename
...
void SHREC::OUTPUT_INFO::SetOutputTriMesh(const bool flag)
{
...
bool SHREC::OUTPUT_INFO::OutputTriMesh() const
{
  return(flag_output_tri_mesh);
}

NUM_TYPE SHREC::OUTPUT_INFO::NumVerticesPerIsopoly() const
{
