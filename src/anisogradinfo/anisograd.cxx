...
/************** DEBUG ***********/
void anisotropic_diff_per_vert_debug
(const ISODUAL3D::ISODUAL_SCALAR_GRID_BASE & scalar_grid,
 const float mu,
 const float lambda,
 const VERTEX_INDEX iv1,
 ...
 ANISO_INFO_TYPE & aniso_info)
{
    ...
    compute_m_d
    ( scalar_grid, gradient_grid, icube, iv1, 0, mX);
    aniso_info.mycopy(mX, aniso_info.mX, DIM3);

    compute_m_d
    ( scalar_grid, gradient_grid, icube, iv1, 1, mY);
    aniso_info.mycopy(mY, aniso_info.mY, DIM3);
    
    compute_m_d
    ( scalar_grid, gradient_grid, icube, iv1, 2, mZ);
    aniso_info.mycopy(mZ, aniso_info.mZ, DIM3);
    
    // compute prev vertex in 0,1,2 direction 
    VERTEX_INDEX prev_vert[DIM3];
    prev_vert[0] = scalar_grid.PrevVertex(iv1, 0);
    aniso_info.prev_vert[0]= prev_vert[0];
    prev_vert[1] = scalar_grid.PrevVertex(iv1, 1);
       aniso_info.prev_vert[1]= prev_vert[1];
    prev_vert[2] = scalar_grid.PrevVertex(iv1, 2);
        aniso_info.prev_vert[2]= prev_vert[2];
    
    compute_m_d
    ( scalar_grid, gradient_grid, icube, prev_vert[0], 0, mX_prev_vert_X);
    aniso_info.mycopy(mX_prev_vert_X, aniso_info.mX_prev_vert_X, DIM3);
    
    compute_m_d
    ( scalar_grid, gradient_grid, icube, prev_vert[1], 1, mY_prev_vert_Y);
        aniso_info.mycopy(mY_prev_vert_Y, aniso_info.mY_prev_vert_Y, DIM3);
    
    compute_m_d
    ( scalar_grid, gradient_grid, icube, prev_vert[2], 2, mZ_prev_vert_Z);
        aniso_info.mycopy(mZ_prev_vert_Z, aniso_info.mZ_prev_vert_Z, DIM3);
    ...
    for (int d=0; d<DIM3; d++) {
        ...
        aniso_info.mycopy(gradHN_d, &(aniso_info.gradHNd[DIM9*d]), DIM9);
        // compute C_d
        GRADIENT_TYPE c[DIM3]={0.0};
        
        compute_c_d(scalar_grid, gradient_grid, iv1, d, c);
        aniso_info.mycopy(c,&(aniso_info.c[DIM3*d]),DIM3);
    ...
    */
    for (int d=0; d<DIM3; d++) 
    {
        ...
        GRADIENT_TYPE   gradHN_d[DIM9]={0.0};
        compute_gradH_d_normals (gradient_grid, prev_vert[d], d,gradHN_d);
         aniso_info.mycopy(gradHN_d, &(aniso_info.gradHNd_prev[DIM9*d]), DIM9);
        // compute C_d
        GRADIENT_TYPE c[DIM3]={0.0};
        compute_c_d(scalar_grid, gradient_grid, prev_vert[d], d, c);
         aniso_info.mycopy(c,&(aniso_info.c_prev[DIM3*d]),DIM3);
        SCALAR_TYPE sum_gradHNd = 0.0, c_square = 0.0;
        ...
        K[d] = sum_gradHNd  - c_square;
        aniso_info.Kprev[d]=K[d];
        compute_g_x(mu, K[d],flag_aniso, gKprev[d]);
        aniso_info.gKprev[d] = gKprev[d];
    }
    ...
    */
    ...
    for (int k=0; k<DIM3; k++) {
        aniso_info.Normals[k]=Normals[k];
    }
    for (int i=0; i<DIM3; i++) {
        wN = wN + Normals[i]*w[i];
    }
    ...
    */
    ...
    for (int i=0; i<DIM3; i++) {
        Normals2[i] = Normals[i]  + lambda *w_dash[i];
        aniso_info.Normals2[i]=Normals2[i];
    }
 ...
 ANISO_INFO_TYPE & aniso_info)
{
    ...
    for (int k=0; k<num_iter; k++) 
    {
        //set anisoinfo_num_iter
        aniso_info.num_iter = k;
        // create a temporary grid for each iteration 
        GRADIENT_GRID temp_gradient_grid;
        temp_gradient_grid.SetSize(scalar_grid, dimension);
        ...
        for (VERTEX_INDEX iv = 0; iv < scalar_grid.NumVertices(); iv++)
        {
            ...
            if (2 <= coord[0] && coord[0]+2 < scalar_grid.AxisSize(0) &&
                ...
                // Store new gradient values in temp_gradient_grid
                anisotropic_diff_per_vert_debug
                (scalar_grid, mu, lambda, iv, flag_aniso, icube, gradient_grid, temp_gradient_grid, aniso_info );
