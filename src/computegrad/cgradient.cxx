...
bool gradients_agree_debug_version
(const GRADIENT_GRID & vertex_gradient_grid,
...
bool gradients_agree(const GRADIENT_GRID & vertex_gradient_grid,
		...
		const VERTEX_INDEX vert1, const INPUT_INFO & io_info)
{
	...
	std::copy(vertex_gradient_grid.VectorPtrConst(vert2),
			vertex_gradient_grid.VectorPtrConst(vert2) + DIM3, &(gradient2[0]));
	IJK::compute_magnitude_3D(gradient2, mag2);
	if (mag2 > 0.0)
	{
		// compute unit vertex gradient
...
//Scalar based prediction debug version

void compute_reliable_gradients_SBP_debug_version (
		const ISODUAL_SCALAR_GRID_BASE & scalar_grid,
		GRADIENT_GRID & vertex_gradient_grid,
		BOOL_GRID & reliable_grid,
		INPUT_INFO & io_info)
 ...
 * neighboring gradients are defined by "reliable_grad_far_dist"
 */
		...
		INPUT_INFO & io_info)
{
	...
	for (VERTEX_INDEX iv=0; iv < scalar_grid.NumVertices(); iv++)
	{
		...
		std::copy(vertex_gradient_grid.VectorPtrConst(iv), vertex_gradient_grid.VectorPtrConst(iv)+DIM3, &(gradient_iv[0]));
		IJK::compute_magnitude_3D(gradient_iv, gradient_iv_mag );
		if (gradient_iv_mag > 0.0)
		{
			//normalize
			for (int l=0;l<DIM3;l++){
			...
			{
				int k = min(io_info.reliable_grad_far_dist, int(coord_iv[d]));
				VERTEX_INDEX prev_vertex = iv - k*scalar_grid.AxisIncrement(d);

				if (gradients_agree(vertex_gradient_grid, gradient_iv, prev_vertex, iv, io_info))
					numAgree++;

				k = min(io_info.reliable_grad_far_dist, (scalar_grid.AxisSize(d)-int(coord_iv[d])-1));
				VERTEX_INDEX next_vertex = iv + k*scalar_grid.AxisIncrement(d);

				if (gradients_agree(vertex_gradient_grid, gradient_iv, next_vertex, iv, io_info))
					numAgree++;
			}
